...
our (
	...
	@ti, # track by index (alias @Audio::Multitrack::Track::by_index)
	%tn, # track by name  (alias %Audio::Multitrack::Track::by_name)
...
use Audio::Multitrack::Iam;    
use Audio::Multitrack::Tkeca_effects; 
use Audio::Multitrack::Track;
use Audio::Multitrack::Bus;    
use Audio::Multitrack::Mark;

# aliases for concise access

*tn = \%Audio::Multitrack::Track::by_name;
*ti = \@Audio::Multitrack::Track::by_index;

# $ti[3]->rw

# print remove_spaces("bulwinkle is a...");

## Class and Object definitions for package 'Audio::Multitrack'

our @ISA; # no anscestors
use Audio::Multitrack::Object qw(mode);
...
sub status_vars {
	serialize -class => 'Audio::Multitrack', -vars => \@status_vars;
}
sub config_vars {
	serialize -class => 'Audio::Multitrack', -vars => \@config_vars;
}
...
sub prepare {  
	...
	$tracker_bus  = Audio::Multitrack::Bus->new(
		name => 'Tracker_Bus',
		groups => [qw(Tracker)],
		tracks => [],
	...
	$master_bus  = Audio::Multitrack::Bus->new(
		name => 'Master_Bus',
		rules  => [ qw(mixer_out mix_link) ],
		groups => ['Master'],
	);
	$mixdown_bus  = Audio::Multitrack::Bus->new(
		name => 'Mixdown_Bus',
		groups => [qw(Mixdown) ],
	...
	$ui = $opts{t} ? Audio::Multitrack::Text->new 
				   : Audio::Multitrack::Graphical->new ;
...
sub load_project {
	...
	dig_ruins() unless $#Audio::Multitrack::Track::by_index > 2;
...
sub initialize_rules {
	...
	$mixer_out = Audio::Multitrack::Rule->new( #  this is the master output
		name			=> 'mixer_out', 
		chain_id		=> 'MixerOut', 
	...
	$mix_down = Audio::Multitrack::Rule->new(
	...
	$mix_link = Audio::Multitrack::Rule->new(
	...
	$mix_setup = Audio::Multitrack::Rule->new(
	...
	$mix_setup_mon = Audio::Multitrack::Rule->new(
	...
	$mon_setup = Audio::Multitrack::Rule->new(
	...
	$rec_file = Audio::Multitrack::Rule->new(
    ...
    $rec_setup = Audio::Multitrack::Rule->new(
	...
	$multi  = Audio::Multitrack::Rule->new(  
...
sub eliminate_loops {
	...
	#my $rule  = grep{ $cooked_id =~ /$_->chain_id/ } Audio::Multitrack::Rule::all_rules();  
	my $rule = $mix_setup; 
...
sub initialize_project_data {
	...
	#print join $/, map{ $_->time} Audio::Multitrack::Mark::all();
 	map{ $_->remove} Audio::Multitrack::Mark::all();
	@marks_data = ();
	...
	$Audio::Multitrack::Group::n = 0; 
	@Audio::Multitrack::Group::by_index = ();
	%Audio::Multitrack::Group::by_name = ();

	$Audio::Multitrack::Track::n = 0; 	# incrementing numeric key
	@Audio::Multitrack::Track::by_index = ();	# return ref to Track by numeric key
	%Audio::Multitrack::Track::by_name = ();	# return ref to Track by name
	%Audio::Multitrack::Track::track_names = (); 

	$master = Audio::Multitrack::Group->new(name => 'Master');
	$mixdown =  Audio::Multitrack::Group->new(name => 'Mixdown');
	$tracker = Audio::Multitrack::Group->new(name => 'Tracker', rw => 'REC');

	#print yaml_out( \%Audio::Multitrack::Track::track_names );


# create magic tracks, we will create their GUI later, after retrieve

	$master_track = Audio::Multitrack::SimpleTrack->new( 
		group => 'Master', 
		name => 'Master',
		rw => 'MON',); # no dir, we won't record tracks


	$mixdown_track = Audio::Multitrack::Track->new( 
		group => 'Mixdown', 
		name => 'Mixdown', 
		rw => 'MON'); 
...
sub add_track {
	...
	my $track = Audio::Multitrack::Track->new(
		name => $name,
		ch_r => $ch_r,
		ch_m => $ch_m,
	);
	...
	# $Audio::Multitrack::chr = 
	
	my $group = $Audio::Multitrack::Group::by_name{$track->group};
	$group->set(rw => 'REC');
...
sub all_chains {
	my @active_tracks = grep { $_->rec_status ne q(OFF) } Audio::Multitrack::Track::all() 
		if Audio::Multitrack::Track::all();
	map{ $_->n} @active_tracks if @active_tracks;
}

sub user_rec_tracks {
	my @user_tracks = Audio::Multitrack::Track::all();
...
sub write_chains {
	...
	for my $full_path (keys %{ $inputs{file} } ) {
		...
		push @input_chains, join ( " ",
					"-a:".$chain_ids,
			 		"-i:".  $Audio::Multitrack::ti[$chain]->modifiers .  $full_path);
 	}
	...
	# write .ewf files
	#
	map{ $_->write_ewf  } Audio::Multitrack::Track::all();
...
sub generate_setup { # create chain setup
	...
	%inputs = %outputs 
			...
			= @output_chains 
			= ();
	my @tracks = Audio::Multitrack::Track::all;
...
sub transport_status {
	my $start  = Audio::Multitrack::Mark::loop_start();
	my $end    = Audio::Multitrack::Mark::loop_end();
...
sub start_heartbeat {
	$event_id{heartbeat} = $new_event->repeat( 3000,
				...
				my ($start, $end);
				$start  = Audio::Multitrack::Mark::loop_start();
				$end    = Audio::Multitrack::Mark::loop_end();
				schedule_wraparound() 
					if $loop_enable 
					and defined $start 
					and defined $end 
...
sub drop_mark {
	...
	return if grep { $_->time == $here } Audio::Multitrack::Mark::all();
	my $mark = Audio::Multitrack::Mark->new( time => $here );
...
sub next_mark {
	...
	$jumps and $jumps--;
	my $here = eval_iam("cs-get-position");
	my @marks = sort { $a->time <=> $b->time } @Audio::Multitrack::Mark::all;
	for my $i ( 0..$#marks ){
...
sub apply_ops {  # in addition to operators in .ecs file
	...
	my $last = scalar @Audio::Multitrack::Track::by_index - 1;
...
map { push @tracks_data, $_->hashref } Audio::Multitrack::Track::all();
...
@marks_data = ();
map { push @marks_data, $_->hashref } Audio::Multitrack::Mark::all();

@groups_data = ();
map { push @groups_data, $_->hashref } Audio::Multitrack::Group::all();
...
sub retrieve_state {
	...
	map {my $g = $_; 
		map{
			$Audio::Multitrack::Group::by_index[$g->{n}]->set($_ => $g->{$_})
			} keys %{$g};
	} @groups_data;
	...
	map{ 
		...
		my $track = Audio::Multitrack::Track->new( %h ) ;
		my $n = $track->n;
		#print "new n: $n\n";
	...
	#print "\n---\n", $tracker->dump;  
	#print "\n---\n", map{$_->dump} Audio::Multitrack::Track::all;# exit; 
	$did_apply and $ui->manifest;
	$debug and print join " ", 
		(map{ ref $_, $/ } @Audio::Multitrack::Track::by_index), $/;
	...
	map{ 
		my %h = %$_; 
		my $mark = Audio::Multitrack::Mark->new( %h ) ;
	} @marks_data;
...
#  unified command processing by command_process 
	...
	$iam_execute = $iam_frame->Button(
			-text => 'Execute',
			-command => sub { print $iam; Audio::Multitrack::Text::command_process( $iam ) }
...
sub oid_gui {
	...
	my @oid_name;
	for my $rule ( Audio::Multitrack::Rule::all_rules ){
		my $name = $rule->name;
...
sub global_version_buttons {
	...
	my @user_track_indices = grep { $_ > 2 } map {$_->n} Audio::Multitrack::Track::all;
...
sub restore_time_marks {
	@_ = discard_object( @_);
# 	map {$_->dumpp} Audio::Multitrack::Mark::all(); 
#	Audio::Multitrack::Mark::all() and 
	map{ $ui->marker($_) } Audio::Multitrack::Mark::all() ; 
...
sub start_heartbeat { Audio::Multitrack::start_heartbeat() }
...
sub refresh {  
	remove_small_wavs();
 	$ui->refresh_group(); 
	map{ $ui->refresh_track($_) } map{$_->n} Audio::Multitrack::Track::all();
}
...
package Audio::Multitrack::Graphical;  ## gui routines
our @ISA = 'Audio::Multitrack';
#use Tk;
#use Audio::Multitrack::Assign qw(:all);
...
sub loop {
	...
	Audio::Multitrack::Text::command_process( $user_input );
	}
#   Term::Shell version
#
# 	my $shell = Audio::Multitrack::Text::OuterShell->new;
# 	my $term = $shell->term();
# 	$term->tkRunning(1);
# 	$shell->cmdloop;
}
...
sub show_versions {
 	print "All versions: ", join " ", @{$Audio::Multitrack::this_track->versions}, $/;
}

sub show_effects {
 	map { 
 		my $op_id = $_;
 		 my $i = $Audio::Multitrack::effect_i{ $Audio::Multitrack::cops{ $op_id }->{type} };
 		 print $op_id, ": " , $Audio::Multitrack::effects[ $i ]->{name},  " ";
 		 my @pnames =@{$Audio::Multitrack::effects[ $i ]->{params}};
			map{ print join " ", 
			 	$pnames[$_]->{name}, 
				$Audio::Multitrack::copp{$op_id}->[$_],'' 
		 	} (0..scalar @pnames - 1);
		 print $/;
 
 	 } @{ $Audio::Multitrack::this_track->ops };
}
sub show_modifiers {
	print "Modifiers: ",$Audio::Multitrack::this_track->modifiers, $/;
}
sub loop {
    package Audio::Multitrack;
...
# 	No TK events in text-only mode
	...
	Audio::Multitrack::Text::command_process( $user_input );
	#print "here we are\n";
 #    use Audio::Multitrack::Text::OuterShell; # not needed, class is present in this file
#      my $shell = Audio::Multitrack::Text::OuterShell->new;
...
package Audio::Multitrack;
        ...
        map {
            ...
            if ($cmd eq 'eval') {
                ...
                $this_track = $tn{$cmd};
                $predicate !~ /^\s*$/ and $Audio::Multitrack::parser->command($predicate);
                ...
                print Audio::Multitrack::eval_iam($user_input), $/ ;
            } else {
                $debug and print "Passing to parser\n", 
                $_, $/;
                #print 1, ref $parser, $/;
                #print 2, ref $Audio::Multitrack::parser, $/;
                # both print
                $parser->command($_) 
            }    
        } @user_input;
...
sub show_tracks {
    no warnings;
    my @tracks = @_;
    map {     push @Audio::Multitrack::format_fields,  
            $_->n,
            $_->name,
            $_->rw,
            $_->rec_status,
...
format STDOUT =
@>>    @<<<<<<<<<    @<<<    @<<<    @>>     @>>>   ~~
splice @Audio::Multitrack::format_fields, 0, 6
.
...
sub catch_run { # 
  ...
  Audio::Multitrack::Text::command_process( $original_command_line );
}
sub catch_help {
  my ($o, $cmd, @args) = @_;
  $debug and print "cmd: $cmd\n";
...
$Audio::Multitrack::AUTOSTUB = 1;
$Audio::Multitrack::RD_HINT = 1;
...
nomodifiers: _nomodifiers end { $Audio::Multitrack::this_track->set(modifiers => ""); }
asdf: 'asdf' { print "hello"}
command: fail
end: /\s*$/ 
end: ';' 
help: _help end { print $Audio::Multitrack::helptext }
help: _help name end { Audio::Multitrack::Text::help($item{name}) }



helpx: 'helpx' end { print "hello_from your command line gramar\n"; }
fail: 'f' end { print "your command line gramar will get a zero\n"; }
exit: _exit end { Audio::Multitrack::save_state(); exit }
create_project: _create_project name end {
	Audio::Multitrack::load_project( 
		name => Audio::Multitrack::remove_spaces($item{name}),
		create => 1,
	);
	print "created project: $Audio::Multitrack::project_name\n";

}

load_project: _load_project name end {
	my $untested = Audio::Multitrack::remove_spaces($item{name});
	print ("Project $untested does not exist\n"), return
	unless -d Audio::Multitrack::join_path Audio::Multitrack::project_root(), $untested; 
	Audio::Multitrack::load_project( name => Audio::Multitrack::remove_spaces($item{name}) );

	print "loaded project: $Audio::Multitrack::project_name\n";
}
save_state: _save_state name end { 
	Audio::Multitrack::save_state( $item{name} ); 
	}
save_state: _save_state end { Audio::Multitrack::save_state() }


get_state: _get_state name end {
	
 	Audio::Multitrack::load_project( 
 		name => $Audio::Multitrack::project_name,
 		settings => $item{name}
 		);
...
getpos: _getpos end {  
	print Audio::Multitrack::d1( Audio::Multitrack::eval_iam q(getpos) ), $/; }
setpos: _setpos value end {
	Audio::Multitrack::eval_iam("setpos $item{value}");
}

add_track: _add_track name end { 
	
	Audio::Multitrack::add_track($item{name}); 
	
}


set_track: _set_track key someval end {
	 $Audio::Multitrack::this_track->set( $item{key}, $item{someval} );
}
dump_track: _dump_track { $Audio::Multitrack::this_track->dumpp }

dump_group: _dump_group { $Audio::Multitrack::tracker->dumpp }

 
remove_track: _remove_track name end {
	$Audio::Multitrack::tn{ $item{name} }->set(hide => 1); }

generate: _generate end { Audio::Multitrack::generate_setup(); }

arm: _arm end { 
	Audio::Multitrack::generate_setup() and Audio::Multitrack::connect_transport(); }

connect: _connect end { Audio::Multitrack::connect_transport(); }

disconnect: _disconnect end { Audio::Multitrack::disconnect_transport(); }


renew_engine: _renew_engine end { Audio::Multitrack::new_engine(); }
engine_status: _engine_status end { print(Audio::Multitrack::eval_iam
q(engine-status));print $/ }

start: _start end { Audio::Multitrack::start_transport(); }
stop: _stop end { Audio::Multitrack::stop_transport(); }

S: _S end { Audio::Multitrack::eval_iam("stop") }
T: _T end { Audio::Multitrack::eval_iam("start") }

show_tracks: _show_tracks end { 	

	Audio::Multitrack::Text::show_tracks ( Audio::Multitrack::Track::all );
	use warnings; 
	no warnings qw(uninitialized); 
	print $/, " " x 7, "Group", " " x 9, $Audio::Multitrack::tracker->rw, " " x 24 , $Audio::Multitrack::tracker->version, $/;
}


modifiers: _modifiers modifier(s) end {
 	 $Audio::Multitrack::this_track->set(modifiers => (join q(,),
	 @{$item{"modifier(s)"}}, q() ))
}

modifiers: _modifiers end { print $Audio::Multitrack::this_track->modifiers, $/; }
	
	
show_chain_setup: _show_chain_setup {
	my $chain_setup;
	Audio::Multitrack::io( Audio::Multitrack::join_path( Audio::Multitrack::project_dir(), $Audio::Multitrack::chain_setup_file) ) > $chain_setup; 
	print $chain_setup, $/;
}

show_io: _show_io { print Audio::Multitrack::yaml_out( \%Audio::Multitrack::inputs ),
Audio::Multitrack::yaml_out( \%Audio::Multitrack::outputs ); }

show_track: _show_track end {
	Audio::Multitrack::Text::show_tracks($Audio::Multitrack::this_track);
	Audio::Multitrack::Text::show_effects();
	Audio::Multitrack::Text::show_versions();
	Audio::Multitrack::Text::show_modifiers();
}
show_track: _show_track name end { 
 	Audio::Multitrack::Text::show_tracks( $Audio::Multitrack::tn{$item{name}} ) if $Audio::Multitrack::tn{$item{name}}
}
show_track: _show_track dd end {  
	Audio::Multitrack::Text::show_tracks( $Audio::Multitrack::ti[$item{dd}] ) if $Audio::Multitrack::ti[$item{dd}]
}
	



group_rec: _group_rec end { $Audio::Multitrack::tracker->set( rw => 'REC') }
group_mon: _group_mon end  { $Audio::Multitrack::tracker->set( rw => 'MON') }
group_off: _group_mute end { $Audio::Multitrack::tracker->set(rw => 'OFF') }

mixdown: _mixdown end { $Audio::Multitrack::mixdown_track->set(rw => 'REC')}
mixplay: _mixplay end { $Audio::Multitrack::mixdown_track->set(rw => 'MON');
						$Audio::Multitrack::tracker->set(rw => 'OFF');
}
mixoff:  _mixoff  end { $Audio::Multitrack::mixdown_track->set(rw => 'OFF');
						$Audio::Multitrack::tracker->set(rw => 'MON')}

record: 'record' end {} 

exit: 'exit' end { Audio::Multitrack::save_state($Audio::Multitrack::state_store_file); exit; }



r: 'r' dd  {	
				$Audio::Multitrack::this_track->set(ch_r => $item{dd});
				$Audio::Multitrack::ch_r = $item{dd};
				print "Input switched to channel $Audio::Multitrack::ch_r.\n";
				
				}
m: 'm' dd  {	
				$Audio::Multitrack::this_track->set(ch_m => $item{dd}) ;
				$Audio::Multitrack::ch_m = $item{dd};
				print "Output switched to channel $Audio::Multitrack::ch_m.\n";
				
				}

off: 'off' end {$Audio::Multitrack::this_track->set(rw => 'OFF'); }
rec: 'rec' end {$Audio::Multitrack::this_track->set(rw => 'REC'); }
mon: 'mon' end {$Audio::Multitrack::this_track->set(rw => 'MON'); }

wav: name { $Audio::Multitrack::this_track = $Audio::Multitrack::tn{$item{name}} if $Audio::Multitrack::tn{$item{name}}  }


set_version: _set_version dd end { $Audio::Multitrack::this_track->set(active => $item{dd})}
 
vol: _vol dd end { $Audio::Multitrack::copp{ $Audio::Multitrack::this_track->vol }->[0] = $item{dd}; 
				Audio::Multitrack::sync_effect_param( $Audio::Multitrack::this_track->vol, 0);
} 
vol: _vol '+' dd end { $Audio::Multitrack::copp{ $Audio::Multitrack::this_track->vol }->[0] += $item{dd};
				Audio::Multitrack::sync_effect_param( $Audio::Multitrack::this_track->vol, 0);
} 
...
vol: _vol end { print $Audio::Multitrack::copp{$Audio::Multitrack::this_track->vol}[0], $/ }

mute: _mute end {

	$Audio::Multitrack::this_track->set(old_vol_level => $Audio::Multitrack::copp{$Audio::Multitrack::this_track->vol}[0])
		if ( $Audio::Multitrack::copp{$Audio::Multitrack::this_track->vol}[0]);  
	$Audio::Multitrack::copp{ $Audio::Multitrack::this_track->vol }->[0] = 0;
	Audio::Multitrack::sync_effect_param( $Audio::Multitrack::this_track->vol, 0);
}
unmute: _unmute end {
	return if $Audio::Multitrack::copp{$Audio::Multitrack::this_track->vol}[0]; 
	return if ! $Audio::Multitrack::this_track->old_vol_level;
	$Audio::Multitrack::copp{$Audio::Multitrack::this_track->vol}[0] = $Audio::Multitrack::this_track->old_vol_level;
	$Audio::Multitrack::this_track->set(old_vol_level => 0);
	Audio::Multitrack::sync_effect_param( $Audio::Multitrack::this_track->vol, 0);
}


unity: _unity end { $Audio::Multitrack::copp{ $Audio::Multitrack::this_track->vol }->[0] = 100;
				Audio::Multitrack::sync_effect_param( $Audio::Multitrack::this_track->vol, 0);
}

pan: _pan dd end { $Audio::Multitrack::copp{ $Audio::Multitrack::this_track->pan }->[0] = $item{dd};
	my $current = $Audio::Multitrack::copp{ $Audio::Multitrack::this_track->pan }->[0];
	$Audio::Multitrack::this_track->set(old_pan_level => $current);
				Audio::Multitrack::sync_effect_param( $Audio::Multitrack::this_track->pan, 0);
...
pan: _pan end { print $Audio::Multitrack::copp{$Audio::Multitrack::this_track->pan}[0], $/ }

pan_right: _pan_right   end { 
	$Audio::Multitrack::copp{ $Audio::Multitrack::this_track->pan }->[0] = 100;
				Audio::Multitrack::sync_effect_param( $Audio::Multitrack::this_track->pan, 0);
}
pan_left:  _pan_left end { $Audio::Multitrack::copp{ $Audio::Multitrack::this_track->pan }->[0] = 0; 
				Audio::Multitrack::sync_effect_param( $Audio::Multitrack::this_track->pan, 0);
}
pan_center: _pan_center end { $Audio::Multitrack::copp{ $Audio::Multitrack::this_track->pan }->[0] = 50   ;
				Audio::Multitrack::sync_effect_param( $Audio::Multitrack::this_track->pan, 0);
}
pan_back:  _pan_back end {
	$Audio::Multitrack::copp{ $Audio::Multitrack::this_track->pan }->[0] = $Audio::Multitrack::this_track->old_pan_level;

}
remove_mark: _remove_mark dd end {
	my @marks = Audio::Multitrack::Mark::all();
	$marks[$item{dd}]->remove if defined $marks[$item{dd}];
}

remove_mark: _remove_mark name end { 
	my $mark = $Audio::Multitrack::Mark::by_name{$item{name}};
	$mark->remove if defined $mark;

}
	
remove_mark: _remove_mark end { 
	return unless (ref $Audio::Multitrack::this_mark) =~ /Mark/;
	$Audio::Multitrack::this_mark->remove;
}
	

mark: _mark end { $Audio::Multitrack::ui->marker( Audio::Multitrack::mark_here() )  }

next_mark: _next_mark end { Audio::Multitrack::next_mark() }

previous_mark: _previous_mark end { Audio::Multitrack::previous_mark() }

loop_enable: _loop_enable someval(s) end {
	my @new_endpoints = @{ $item{"someval(s)"}}; 
	
	$Audio::Multitrack::loop_enable = 1;
	@Audio::Multitrack::loop_endpoints = (@new_endpoints, @Audio::Multitrack::loop_endpoints); 
	@Audio::Multitrack::loop_endpoints = @Audio::Multitrack::loop_endpoints[0,1];
}
loop_disable: _loop_disable end {
	$Audio::Multitrack::loop_enable = 0;
}
	
name_mark: _name_mark name end {$Audio::Multitrack::this_mark->set_name( $item{name}) }

list_marks: _list_marks end { 
	my $i = 0;
	map{ print( $_->time == $Audio::Multitrack::this_mark->time ? q(*) : q()
	,join " ", $i++, sprintf("%.1f", $_->time), $_->name, $/)  } 
		  
		  @Audio::Multitrack::Mark::all;
	my $start = my $end = "undefined";
	print "now at ", sprintf("%.1f", Audio::Multitrack::eval_iam "getpos"), $/;
...
remove_effect: _remove_effect op_id(s) end {
	...
	map{ print "removing effect id: $_\n"; Audio::Multitrack::remove_effect( $_ )
	} grep { $_ }  @{ $item{"op_id(s)"}} ;
...
my $code = $item{name};
if ( $Audio::Multitrack::effect_i{$code} ) {} 
elsif ( $Audio::Multitrack::effect_j{$code} ) { $code = $Audio::Multitrack::effect_j{$code} }
else { warn "effect code not found: $code\n"; return }
print "code: ", $code, $/;
	my %p = (
		chain => $Audio::Multitrack::this_track->n,
		values => $item{"value(s?)"},
		type => $code,
		);
		print "adding effect\n";
		
	Audio::Multitrack::add_effect( \%p );
}
...
modify_effect: _modify_effect op_id parameter sign(?) value end {
		...
		if ($item{"sign(?)"} and @{ $item{"sign(?)"} }) {
			$new_value = 
 			eval (join " ",
 				$Audio::Multitrack::copp{$item{op_id}}->[$item{parameter}], 
 				@{$item{"sign(?)"}},
 				$item{value});
		}
			
	Audio::Multitrack::effect_update_copp_set( 
		$Audio::Multitrack::cops{ $item{op_id} }->{chain}, 
		$item{op_id}, 
		$item{parameter}, 
		$new_value);
...
group_version: _group_version end { 
	use warnings;
	no warnings qw(uninitialized);
	print $Audio::Multitrack::tracker->version, $/ }
group_version: _group_version dd end { $Audio::Multitrack::tracker->set( version => $item{dd} )}


list_versions: _list_versions end { 
	print join " ", @{$Audio::Multitrack::this_track->versions}, $/;
}

ladspa_register: _ladspa_register end { print Audio::Multitrack::eval_iam("ladspa-register") }
preset_register: _preset_register end { print Audio::Multitrack::eval_iam("preset-register") }
ctrl_register: _ctrl_register end { print Audio::Multitrack::eval_iam("ctrl-register") }
project_name: _project_name end { print "project name: ", $Audio::Multitrack::project_name, $/ }
...
open STDERR, ">&SAVERR";
#select STDOUT; $| = 1;
# Audio::Multitrack::Text::OuterShell::create_help_subs();
#
