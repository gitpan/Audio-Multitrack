...
# ---------- Track -----------
use strict;
package Audio::Multitrack::Track;
our $VERSION = 1.0;
our ($debug);
local $debug = 0;
#use Exporter qw(import);
#our @EXPORT_OK = qw(track);
use Audio::Multitrack::Assign qw(join_path);
use Carp;
use IO::All;
use vars qw($n %by_name @by_index %track_names);
use Audio::Multitrack::Wav;
our @ISA = 'Audio::Multitrack::Wav';
...
sub new {
	...
	#print "names used: ", Audio::Multitrack::yaml_out( \%track_names );
	$by_index[$n] = $object;
	$by_name{ $object->name } = $object;
	
	Audio::Multitrack::add_volume_control($n);
	Audio::Multitrack::add_pan_control($n);

	#my $group = $Audio::Multitrack::Group::by_name{ $object->group }; 

	# create group if necessary
	#defined $group or $group = Audio::Multitrack::Group->new( name => $object->group );
	#my @existing = $group->tracks ;
	#$group->set( tracks => [ @existing, $object->name ]);
	$Audio::Multitrack::this_track = $object;
	$object;
	
}

sub dir { Audio::Multitrack::this_wav_dir() } # replaces dir field
...
sub write_ewf {
	$Audio::Multitrack::debug2 and print "&write_ewf\n";
	my $track = shift;
...
package Audio::Multitrack::SimpleTrack; # used for Master track
our @ISA = 'Audio::Multitrack::Track';
use Audio::Multitrack::Object qw( 	name
						dir
						active
...
package Audio::Multitrack::Group;
our $VERSION = 1.0;
#use Exporter qw(import);
...
sub tracks { # returns list of tracks in group 
	...
	my $group = shift;
	my @all = Audio::Multitrack::Track::all;
	# map {print "type: ", ref $_, $/} Audio::Multitrack::Track::all; 
	map{ $_->name } grep{ $_->group eq $group->name } Audio::Multitrack::Track::all();
}
...
package Audio::Multitrack::Op;
our $VERSION = 0.5;
our @ISA;
use Audio::Multitrack::Object qw(	op_id 
					chain_id 
					effect_id
					parameters
					subcontrollers
