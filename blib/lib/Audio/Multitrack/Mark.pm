...
# ----------- Mark ------------
package Audio::Multitrack::Mark;
our $VERSION = 1.0;
use Carp;
our @ISA;
use vars qw($n %by_name @all  %used_names);
use Audio::Multitrack::Object qw( 
				 name 
                 time
				 active
				 );
...
sub new {
	...
	push @all, $object;
	$Audio::Multitrack::this_mark = $object;
...
sub jump_here {
	...
	carp("you attempted to futz $mark, which is not a Mark\n"),
		return unless (ref $mark) =~ /Mark/;
	Audio::Multitrack::eval_iam( "setpos " . $mark->time);
	$Audio::Multitrack::this_mark = $mark;
}
sub remove {
	my $mark = shift;
	...
	$Audio::Multitrack::debug and warn "marks found: ",scalar @all, $/;
...
sub next { 
	...
	carp("you attempted to futz $mark, which is not a Mark\n"),
		return unless (ref $mark) =~ /Mark/;
	Audio::Multitrack::next_mark();
}
sub previous {
	my $mark = shift; 
	carp("you attempted to futz $mark, which is not a Mark\n"),
		return unless (ref $mark) =~ /Mark/;
	Audio::Multitrack::previous_mark();
}
...
sub loop_start { 
	my @points = sort { $a <=> $b } 
	grep{ $_ } map{ mark_time($_)} @Audio::Multitrack::loop_endpoints[0,1];
	#print "points @points\n";
	$points[0];
}
sub loop_end {
...
sub mark_time {
	...
	if ($tag =~ /\./) { # we assume raw time if decimal
		...
		$mark = $Audio::Multitrack::Mark::all[$tag];
	} else {
		#print "mark name found\n";
		$mark = $Audio::Multitrack::Mark::by_name{$tag};
	}
